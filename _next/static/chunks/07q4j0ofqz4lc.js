...
]`,{temp:.3,max:12e3,webSearch:!0});try{let e=m(i);return Array.isArray(e)?e:[e]}catch{console.warn("[HookFlow] 후킹 1차 파싱 실패, 웹검색 없이 재시도");let a=m(await g("SNS 마케팅 작가. 순수 JSON 배열만. 설명 금지.",`"${e.title}" ${n[t]||t} 대본 ${r}개. 형식:
[{"headline":"15자","subheadline":"30자","bodyPoints":["","","","",""],"callToAction":"","targetAudience":""}]`,{temp:.5,max:3e3}));return Array.isArray(a)?a:[a]}}async function S(e,t=5){let r=m(await g(`당신은 한국 SNS 바이럴 콘텐츠 작가입니다.
