...
]`,{temp:.3,max:12e3,webSearch:!0});try{let e=m(o);return Array.isArray(e)?e:[e]}catch{console.warn("[HookFlow] 후킹 1차 파싱 실패, 웹검색 없이 재시도");let n=m(await p("SNS 마케팅 작가. 순수 JSON 배열만. 설명 금지.",`"${e.title}" ${a[t]||t} 대본 ${r}개. 형식:
[{"headline":"15자","subheadline":"30자","bodyPoints":["","","","",""],"callToAction":"","targetAudience":""}]`,{temp:.5,max:3e3}));return Array.isArray(n)?n:[n]}}async function w(e,t=5){let r=m(await p(`당신은 한국 SNS 바이럴 콘텐츠 작가입니다.
...
]`,{temp:.7,max:8e3}));return Array.isArray(r)?r:[r]}async function v(e,t="instagram"){if(!g())return e.map(()=>"");let r={instagram:"정사각형 1:1",tiktok:"세로 9:16",facebook:"가로 16:9"},a=Array(e.length).fill("");for(let n=0;n<e.length;n+=2){let o=e.slice(n,n+2).map((o,i)=>{let s=n+i,l=String(o.title||""),c=String(o.subtitle||""),d=String(o.textEmphasis||l),u=String(o.imagePrompt||""),f=String(o.colorScheme||""),g=s===e.length-1;return h(`한국 SNS 콘텐츠 이미지 생성.
...
${a?"CTA 느낌, 결심하는 인물":r?"스크롤 멈추는 충격적 장면":"진지한 정보 전달"}`;return await h(c)||""}async function $(e){await s(e),localStorage.removeItem("hookflow_history")}async function A(){let e=new Date().toLocaleDateString("ko-KR"),t=await p(`한국/글로벌 YouTube\xb7SNS 바이럴 영상 분석 전문가.
