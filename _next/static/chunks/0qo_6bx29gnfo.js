...
]`,{temp:.3,max:12e3,webSearch:!0});try{let e=m(o);return Array.isArray(e)?e:[e]}catch{console.warn("[HookFlow] 후킹 1차 파싱 실패, 웹검색 없이 재시도");let a=m(await p("SNS 마케팅 작가. 순수 JSON 배열만. 설명 금지.",`"${e.title}" ${n[t]||t} 대본 ${r}개. 형식:
[{"headline":"15자","subheadline":"30자","bodyPoints":["","","","",""],"callToAction":"","targetAudience":""}]`,{temp:.5,max:3e3}));return Array.isArray(a)?a:[a]}}async function w(e,t=5){let r=m(await p(`당신은 한국 SNS 바이럴 콘텐츠 작가입니다.
...
]`,{temp:.7,max:8e3}));return Array.isArray(r)?r:[r]}async function v(e,t="instagram"){if(!g())return e.map(()=>"");let r={instagram:"정사각형 1:1",tiktok:"세로 9:16",facebook:"가로 16:9"},n=Array(e.length).fill("");for(let a=0;a<e.length;a+=2){let o=e.slice(a,a+2).map((o,i)=>{let s=a+i,l=String(o.title||""),c=String(o.subtitle||""),d=String(o.textEmphasis||l),u=String(o.imagePrompt||""),f=String(o.colorScheme||""),g=s===e.length-1;return h(`한국 SNS 콘텐츠 이미지 생성.
...
출력 전 검증: 이미지 내 모든 한국어가 "${d}"와 "${c}"와 정확히 일치하는지 확인.`).then(e=>{n[s]=e||""}).catch(e=>{console.warn(`[Image ${s+1}] 실패:`,e),n[s]=""})});await Promise.allSettled(o),a+2<e.length&&await new Promise(e=>setTimeout(e,3e3))}return n}async function j(e,t="instagram",r=!1,n=!1){let a=String(e.title||""),o=String(e.subtitle||""),i=String(e.textEmphasis||a),s=String(e.imagePrompt||""),l=String(e.colorScheme||""),c=`한국 SNS 콘텐츠 이미지.
장면: ${s}
색상: ${l||"다크 + 보라"}
상단/하단 한국어 텍스트: "${i}"${o?` / "${o}"`:""}
...
${n?"CTA 느낌, 결심하는 인물":r?"스크롤 멈추는 충격적 장면":"진지한 정보 전달"}`;return await h(c)||""}async function $(e){await s(e),localStorage.removeItem("hookflow_history")}async function A(){let e=new Date().toLocaleDateString("ko-KR"),t=await p(`한국/글로벌 YouTube\xb7SNS 바이럴 영상 분석 전문가.
...
|---|---|---|---|---|---|---|
`,n.forEach((e,r)=>{t+=`| ${r+1} | ${e.title} | ${e.channel} | ${e.viewsStr} | ${e.engagementRate}% | $${e.estimatedCPM} | $${e.estimatedRevenue.toLocaleString()} |
`}),t+=`
`;for(let[r,n]of(t+=`
## 2️⃣ 성과 분석
...
`,Object.entries(e.hookPatternDistribution)))t+=`- **${r}**: ${n}개
`;for(let[r,n]of(t+=`
### 📱 Shorts vs 장편
...
`,Object.entries(e.regionDistribution)))t+=`- ${r}: ${n}개
`;return t+=`
## 4️⃣ 다음 주 트렌드 예측
