...
[{"headline":"","subheadline":"","bodyPoints":["","","","",""],"callToAction":"","targetAudience":""}]`,{temp:.3,max:16e3,webSearch:!0}));return Array.isArray(o)?o:[o]}async function v(e,t=8){let r=m(await g(`SNS 콘텐츠 작가. 초보자도 이해하기 쉽게 친절하게 작성. 한국어 맞춤법 완벽. JSON만 반환.
...
]`,{temp:.6,max:6e3}));return Array.isArray(r)?r:[r]}async function S(e,t="instagram"){if(!p())return e.map(()=>"");let r={instagram:"정사각형 1:1",tiktok:"세로 9:16",facebook:"가로 16:9"},n=Array(e.length).fill("");for(let a=0;a<e.length;a+=2){let o=e.slice(a,a+2).map((o,i)=>{let s=String(o.title||""),l=String(o.subtitle||""),c=String(o.body||""),d=a+i===0,u=a+i===e.length-1;return h(`${d?`스크롤 멈추는 한국 SNS 메인 이미지.
