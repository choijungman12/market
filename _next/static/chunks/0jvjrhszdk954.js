...
[{"headline":"15자","subheadline":"30자","bodyPoints":["","","","",""],"callToAction":"","targetAudience":""}]`,i=await p(a?"한국 SNS 마케팅 작가. 웹 검색 후 JSON만 출력. 설명 금지.":"한국 SNS 마케팅 작가. JSON만 출력. 설명 금지. 오타 금지.",o,{temp:.6,max:a?12e3:3e3,webSearch:a});try{let e=m(i);return Array.isArray(e)?e:[e]}catch{console.warn("[HookFlow] 후킹 1차 파싱 실패, 웹검색 없이 재시도");let a=m(await p("SNS 마케팅 작가. 순수 JSON 배열만. 설명 금지.",`"${e.title}" ${n[t]||t} 대본 ${r}개. 형식:
...
${a?"CTA 느낌, 결심하는 인물":r?"스크롤 멈추는 충격적 장면":"진지한 정보 전달"}`;return await g(c)||""}async function N(e){await s(e),localStorage.removeItem("hookflow_history")}async function $(){let e=new Date().toLocaleDateString("ko-KR"),t=await p(`한국/글로벌 YouTube\xb7SNS 바이럴 영상 분석 전문가.
